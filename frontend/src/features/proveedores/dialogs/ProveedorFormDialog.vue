...
const draft = defineModel<ProveedorDraft>('draft', { required: true })

const formRef = ref<InstanceType<typeof ProveedorForm>>()
</script>

<template>
  <div v-if="visible" class="fixed inset-0 z-[99999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-xl flex flex-col overflow-hidden">
      <div class="flex items-center justify-between px-6 py-4 border-b border-default surface-header">
        <div>
          <h3 class="text-[14px] font-bold text-heading">{{ modo === 'nuevo' ? 'Nuevo Proveedor' : 'Editar Proveedor' }}</h3>
          <p class="text-[11px] text-muted mt-0.5">Complete los campos requeridos</p>
        </div>
