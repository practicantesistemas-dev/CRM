      ...
      <table class="w-full min-w-[820px]">
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="e in rows" :key="e.id" class="hover:bg-slate-50/60 transition-colors group">
            ...
            <td class="px-5 py-3.5 text-right">
              <div class="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button @click="emit('borrar', e)" class="w-7 h-7 rounded-lg bg-slate-100 hover:bg-red-50 hover:text-red-600 text-slate-500 flex items-center justify-center transition-all" title="Eliminar">
                  <Trash2 :size="12" />
                </button>
              </div>
            </td>
          ...
          <tr v-if="rows.length === 0">
            <td colspan="7" class="text-center py-16 text-slate-400 text-[12px]">No se encontraron empresas.</td>
          </tr>
        </tbody>
      </table>
    </div>
