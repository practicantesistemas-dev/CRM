...
defineProps<{ empresa: Empresa | null; items: HistorialItem[]; cargando?: boolean }>()
  ...
  <div v-if="visible" class="fixed inset-0 z-[99999]" @click.self="visible = false">
    ...
    <div class="fixed right-0 top-0 h-full w-full max-w-sm bg-white shadow-2xl flex flex-col z-10">
      ...
      <div class="flex-1 overflow-y-auto p-5 space-y-3">
        ...
        <p v-else-if="items.length === 0" class="text-center text-[11px] text-slate-400 py-8">Sin actividades registradas.</p>
        <template v-else>
      ...
      <div class="px-5 py-4 border-t border-slate-200">
        <button @click="visible = false; emit('registrar')"
          class="w-full h-9 rounded-lg bg-[#2447F9] text-white text-[11px] font-bold hover:bg-[#1D3DD9] transition-all flex items-center justify-center gap-1.5">
