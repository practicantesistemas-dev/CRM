...
<template>
  <div v-if="visible" class="fixed inset-0 z-[999999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" @click.self="cerrar">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-amber-50 shrink-0">
        <div>
          <h3 class="text-[14px] font-bold text-[#0F172A] flex items-center gap-2">
            <RefreshCw :size="15" class="text-amber-600" />
      ...
      <div class="overflow-y-auto flex-1 p-6">
        <div class="mb-5 flex items-start gap-2.5 bg-amber-50 border border-amber-200 rounded-xl px-3.5 py-3">
        ...
        <div v-if="props.error" class="mb-4 flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-3 py-2">
          <AlertCircle :size="13" class="text-red-500 shrink-0" />
          <p class="text-[11px] text-red-600 font-medium">{{ props.error }}</p>
        </div>

        <ReemplazoPersonaForm ref="formRef" v-model="draft" @valid-submit="emit('submit')" />

        <label class="mt-6 flex items-start gap-2.5 bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-3 cursor-pointer">
      ...
      <div class="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-200 bg-[#F8FAFC] shrink-0">
