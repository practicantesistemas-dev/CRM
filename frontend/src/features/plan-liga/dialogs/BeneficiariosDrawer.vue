...
import { X, Users, AlertCircle, Plus } from 'lucide-vue-next'
    ...
    <div class="fixed right-0 top-0 h-full w-full max-w-lg bg-white shadow-2xl flex flex-col z-10">
      ...
      <div class="px-5 py-3 border-b border-slate-100 shrink-0">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-[12px] font-bold" :class="activosActual >= CUPO_MAXIMO ? 'text-[#EC4899]' : 'text-[#059669]'">{{ activosActual }} / {{ CUPO_MAXIMO }} activos</span>
        </div>
        <CuposIndicador :activos="activosActual" variant="bar" />
        <div v-if="errLimite" class="mt-2 flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-3 py-2">
          ...
          <p class="text-[11px] text-red-600 font-medium">El titular ya cuenta con el máximo permitido de <strong>{{ CUPO_MAXIMO }} beneficiarios activos</strong>.</p>
      ...
      <div class="px-4 py-4 border-t border-slate-200 shrink-0">
        <button @click="emit('agregar-nuevo')" :disabled="!puedeAgregar"
          ...
          <Plus :size="14" />
          <span v-if="puedeAgregar">Agregar beneficiario ({{ activosActual }}/{{ CUPO_MAXIMO }})</span>
          <span v-else>Límite alcanzado ({{ CUPO_MAXIMO }}/{{ CUPO_MAXIMO }})</span>
        </button>
      </div>
    </div>
  </div>
