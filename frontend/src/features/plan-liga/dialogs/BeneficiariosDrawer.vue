  ...
  <div v-if="visible" class="fixed inset-0 z-[99999]">
    ...
    <div class="fixed right-0 top-0 h-full w-full max-w-lg bg-white dark:bg-slate-800 shadow-2xl flex flex-col z-10">
      <div class="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-slate-700 bg-[#F8FAFC] dark:bg-slate-900 shrink-0">
        <div>
          <h3 class="text-[13px] font-bold text-[#0F172A] dark:text-slate-100 flex items-center gap-2"><Users :size="15" class="text-[#EC4899]" />Beneficiarios</h3>
          <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{{ titular?.nombre }}</p>
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-3">
        <div v-if="cargando" class="flex items-center justify-center py-10 text-slate-400 dark:text-slate-500 text-[12px] gap-2">
