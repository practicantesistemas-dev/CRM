    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
      <template v-if="paso === 'formulario'">
        <div class="p-6 text-center">
          <div class="w-12 h-12 rounded-2xl bg-[#EEF2FF] flex items-center justify-center mx-auto mb-4">
            <ArrowLeftRight :size="20" class="text-[#2447F9]" />
          </div>
          <h3 class="text-[14px] font-bold text-[#0F172A] mb-1">Cambiar de titular</h3>
          <p class="text-[12px] text-slate-500">
            Mueve a <strong>{{ props.nombreActual }}</strong> a otro titular, conservando su registro (plan, historial, etc.).
          </p>
          <div class="text-left mt-4">
            <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wide mb-1.5">Documento del nuevo titular</label>
            <input
              v-model="documento"
              type="text"
              placeholder="Número de documento"
              class="w-full h-10 px-3 rounded-lg border border-slate-200 text-[12px] outline-none focus:border-[#2447F9]"
      ...
      <template v-else>
        <div class="p-6 text-center">
          ...
          <h3 class="text-[14px] font-bold text-[#0F172A] mb-1">¿Confirmar el cambio?</h3>
          <p class="text-[12px] text-slate-500">
            Vas a mover a <strong>{{ props.nombreActual }}</strong> al titular con documento <strong>{{ documento }}</strong>.
            Esta acción no se puede deshacer.
          </p>
          <p v-if="props.error" class="text-[11px] text-red-600 font-medium mt-3">{{ props.error }}</p>
        </div>
        <div class="flex gap-2 px-6 pb-6">
          <button @click="cerrar" :disabled="props.guardando" class="flex-1 h-9 rounded-lg border border-slate-200 bg-white text-[11px] font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-60 disabled:cursor-not-allowed transition-all">
            Cancelar
          </button>
