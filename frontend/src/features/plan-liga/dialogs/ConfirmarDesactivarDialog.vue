    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
      <div class="p-6 text-center">
        <div class="w-12 h-12 rounded-2xl bg-red-50 dark:bg-red-950/50 flex items-center justify-center mx-auto mb-4">
          <ToggleLeft :size="20" class="text-red-500 dark:text-red-400" />
        </div>
        <h3 class="text-[14px] font-bold text-[#0F172A] dark:text-slate-100 mb-1">{{ props.titulo }}</h3>
        <p class="text-[12px] text-slate-500 dark:text-slate-400">¿Confirma que desea desactivar a <strong>{{ props.nombre }}</strong>?</p>
        <p v-if="props.error" class="text-[11px] text-red-600 dark:text-red-400 font-medium mt-2">{{ props.error }}</p>
      </div>
