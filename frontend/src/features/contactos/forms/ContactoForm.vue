...
<template>
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <div>
      ...
      <input v-model="draft.documento" @blur="tocar('documento')" placeholder="Ej: 10293844" class="w-full h-10 px-4 rounded-lg border bg-slate-50 dark:bg-slate-900 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:bg-white dark:focus:bg-slate-800 transition-all" :class="fieldStateClass(esVisible('documento') && !!errors.documento, esVisible('documento') && !errors.documento && !!draft.documento, 'border-slate-200 dark:border-slate-600 focus:border-[#2447F9]')" />
      <FieldError :message="esVisible('documento') ? errors.documento : undefined" />
    </div>
    <div>
      <label class="block text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Correo electrónico</label>
      <div class="relative"><Mail :size="13" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
        <input v-model="draft.correo" @blur="tocar('correo')" type="email" placeholder="correo@empresa.com" class="w-full h-10 pl-9 pr-4 rounded-lg border bg-slate-50 dark:bg-slate-900 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:bg-white dark:focus:bg-slate-800 transition-all" :class="fieldStateClass(esVisible('correo') && !!errors.correo, esVisible('correo') && !errors.correo && !!draft.correo, 'border-slate-200 dark:border-slate-600 focus:border-[#2447F9]')" /></div>
    ...
    <div>
      ...
      <div class="relative"><Phone :size="13" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
        <input v-model="draft.telefono" @blur="tocar('telefono')" placeholder="300-000-0000" class="w-full h-10 pl-9 pr-4 rounded-lg border bg-slate-50 dark:bg-slate-900 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:bg-white dark:focus:bg-slate-800 transition-all" :class="fieldStateClass(esVisible('telefono') && !!errors.telefono, esVisible('telefono') && !errors.telefono && !!draft.telefono, 'border-slate-200 dark:border-slate-600 focus:border-[#2447F9]')" /></div>
      <FieldError :message="esVisible('telefono') ? errors.telefono : undefined" />
    </div>
    <div>
      <label class="block text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Empresa <span class="font-normal normal-case text-slate-400 dark:text-slate-500">(opcional)</span></label>
      <div class="relative">
        <Building2 :size="13" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 z-10" />
    ...
    <div>
      <label class="block text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Cargo</label>
      <div class="relative"><Briefcase :size="13" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
        <input v-model="draft.cargo" placeholder="Ej: Gerente Comercial" class="w-full h-10 pl-9 pr-4 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:border-[#2447F9] focus:bg-white dark:focus:bg-slate-800 transition-all" /></div>
    </div>
    <div>
    ...
    <div>
      <label class="block text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Tipo de contacto</label>
      <select v-model="draft.tipoContacto" class="w-full h-10 px-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:border-[#2447F9] focus:bg-white dark:focus:bg-slate-800 transition-all cursor-pointer">
    ...
    <div>
      ...
      <select v-model="draft.sexo" class="w-full h-10 px-3 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:border-[#2447F9] focus:bg-white dark:focus:bg-slate-800 transition-all cursor-pointer">
    ...
    <div class="sm:col-span-2">
      <label class="block text-[11px] font-bold text-slate-600 dark:text-slate-300 mb-1.5 uppercase tracking-wide">Etiquetas <span class="font-normal normal-case text-slate-400 dark:text-slate-500">(opcional)</span></label>

      <!-- Seleccionadas -->
      <div v-if="draft.etiquetas.length" class="flex gap-2 mb-2 flex-wrap">
        <span v-for="tag in draft.etiquetas" :key="tag.id" class="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-1 rounded-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-600" :style="{ color: tag.color }">
      ...
      <div v-if="cargandoEtiquetas" class="flex items-center gap-1.5 text-[11px] text-slate-400 dark:text-slate-500 mb-2"><Loader2 :size="12" class="animate-spin" />Cargando etiquetas...</div>
      ...
      <button v-if="!mostrarNuevaEtiqueta" type="button" @click="mostrarNuevaEtiqueta = true"
        class="flex items-center gap-1 h-8 px-3 rounded-lg border border-dashed border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-[11px] font-semibold text-slate-500 dark:text-slate-400 hover:border-[#2447F9] hover:text-[#2447F9] transition-all">
        <Plus :size="11" /> Nueva etiqueta
      </button>
      <div v-else class="flex items-center gap-2">
        <div class="relative flex-1"><Tag :size="13" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
          <input v-model="nuevaEtiquetaNombre" placeholder="Nombre de la etiqueta" @keydown.enter.prevent="crearYSeleccionarEtiqueta"
            class="w-full h-9 pl-9 pr-4 rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:border-[#2447F9] focus:bg-white dark:focus:bg-slate-800 transition-all" /></div>
        <input v-model="nuevaEtiquetaColor" type="color" title="Color de la etiqueta" class="h-9 w-9 rounded-lg border border-slate-200 dark:border-slate-600 cursor-pointer" />
        ...
        <button type="button" @click="mostrarNuevaEtiqueta = false; nuevaEtiquetaNombre = ''" class="h-9 px-2 rounded-lg text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300"><X :size="14" /></button>
      </div>
      <p v-if="errorCrearEtiqueta" class="flex items-center gap-1.5 text-[11px] text-red-600 dark:text-red-400 font-medium mt-2"><AlertCircle :size="12" />{{ errorCrearEtiqueta }}</p>
    </div>
  </div>
</template>
