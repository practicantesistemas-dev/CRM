  ...
  <div v-if="visible" class="fixed inset-0 z-[99999] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" @click.self="visible = false">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-[#F8FAFC] dark:bg-slate-900">
        <div>
          <h3 class="text-[14px] font-bold text-[#0F172A] dark:text-slate-100">{{ modo === 'nuevo' ? 'Nuevo Contacto' : 'Editar Contacto' }}</h3>
          <p class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">Complete todos los campos requeridos</p>
        </div>
      ...
      <div class="flex items-center justify-end gap-2 px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-[#F8FAFC] dark:bg-slate-900">
        <button @click="visible = false" :disabled="guardando" class="h-9 px-5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-[11px] font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-60 disabled:cursor-not-allowed transition-all">Cancelar</button>
