  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div class="sm:col-span-2">
      ...
      <div class="relative">
        ...
        <input v-model="draft.nombre" @blur="tocar('nombre')" placeholder="Ej: Plan Liga Empresarial" class="w-full h-10 pl-9 pr-4 rounded-lg border bg-slate-50 dark:bg-slate-900 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:bg-white dark:focus:bg-slate-800 transition-all" :class="fieldStateClass(esVisible('nombre') && !!errors.nombre, esVisible('nombre') && !errors.nombre && !!draft.nombre, 'border-slate-200 dark:border-slate-600 focus:border-[#2447F9]')" />
      </div>
      <FieldError :message="esVisible('nombre') ? errors.nombre : undefined" />
    </div>
    <div>
      <label class="block text-[11px] font-bold text-body mb-1.5 uppercase tracking-wide">Código *</label>
      <div class="relative">
        <Hash :size="13" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
        <input v-model="draft.codigo" @blur="tocar('codigo')" placeholder="PLE-001" class="w-full h-10 pl-9 pr-4 rounded-lg border bg-slate-50 dark:bg-slate-900 text-[12px] text-slate-900 dark:text-slate-100 outline-none focus:bg-white dark:focus:bg-slate-800 transition-all" :class="fieldStateClass(esVisible('codigo') && !!errors.codigo, esVisible('codigo') && !errors.codigo && !!draft.codigo, 'border-slate-200 dark:border-slate-600 focus:border-[#2447F9]')" />
      </div>
      <FieldError :message="esVisible('codigo') ? errors.codigo : undefined" />
    </div>
    <div>
      <label class="block text-[11px] font-bold text-body mb-1.5 uppercase tracking-wide">Categoría</label>
      <input v-model="draft.categoria" placeholder="Ej: Plan Liga" class="w-full h-10 px-4 rounded-lg input-surface text-[12px] outline-none focus:border-[#2447F9] focus:bg-white dark:focus:bg-slate-800 transition-all" />
    </div>
    <div>
      <label class="block text-[11px] font-bold text-body mb-1.5 uppercase tracking-wide">Tipo</label>
      <select v-model="draft.tipo" class="w-full h-10 px-3 rounded-lg input-surface text-[12px] outline-none focus:border-[#2447F9] focus:bg-white dark:focus:bg-slate-800 transition-all cursor-pointer">
    ...
    <div>
      ...
      <div class="relative">
        ...
        <select v-model="draft.responsable" class="w-full h-10 pl-9 pr-4 rounded-lg input-surface text-[12px] outline-none focus:border-[#2447F9] focus:bg-white dark:focus:bg-slate-800 transition-all cursor-pointer">
    ...
    <div>
      <label class="block text-[11px] font-bold text-body mb-1.5 uppercase tracking-wide">Valor Potencial</label>
      <input v-model="draft.valorPotencial" placeholder="Ej: $10.000.000" class="w-full h-10 px-4 rounded-lg input-surface text-[12px] outline-none focus:border-[#2447F9] focus:bg-white dark:focus:bg-slate-800 transition-all" />
    </div>
  </div>
</template>
