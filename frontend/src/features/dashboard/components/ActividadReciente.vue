    ...
    <div v-else class="divide-y divide-slate-50">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          ...
          <p v-if="act.descripcion" class="text-[11px] text-slate-600 mt-1 line-clamp-2">{{ act.descripcion }}</p>
        </div>
        <span class="text-[10px] text-slate-400 flex-shrink-0 whitespace-nowrap mt-0.5">{{ act.fechaHora }}</span>
      </div>
    </div>
  </div>
</template>
